    ...
    <section class="mt-20 lg:mt-8">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    @foreach (["Batsman", "Bowler", "All Rounder", "Wicket Keeper"] as $role)
                        <button wire:click="setFilter('{{ $role }}')"
                            ...
                            {{ $filter === $role
                                ...
                                : 'bg-white/70 text-gray-600 backdrop-blur-md border border-white/40 hover:bg-teal-50 hover:text-teal-700' }}">
                            {{ $role }}
                        </button>
                    @endforeach
           ...
           @forelse ($players ?? [] as $player)
               ...
               <!-- ===== Player Card ===== -->
               <div
                   class="group relative h-80 overflow-hidden rounded-3xl
                          bg-white/5 backdrop-blur-xl
                          border border-white/10
                          shadow-[0_20px_50px_rgba(0,0,0,0.25)]
                          transition-all duration-500
                          hover:-translate-y-1 hover:shadow-[0_30px_70px_rgba(0,0,0,0.35)]">
       
                   <!-- Player Tag (Top Right Corner) -->
                   @if (!empty($player->player_tag))
                       <div
                           class="absolute right-4 top-4 z-50
                                  rounded-full bg-teal-500/90
                                  px-3 py-1 text-[11px]
                   ...
                   <div
                       class="absolute inset-0 flex flex-col items-center justify-center
                              bg-black/85 p-6 text-center
                              opacity-0 transition-all duration-500
                              group-hover:opacity-100">
       
                       <h3 class="mb-3 text-sm font-semibold tracking-wide text-white uppercase">
                       ...
                       <a href="{{ route('frontend.profile', $player->slug) }}"
                          class="rounded-full bg-teal-500 px-5 py-1.5
                   ...
                   <div
                       ...
                       <!-- Player Name -->
                       <span
                           class="truncate text-[13px] font-semibold tracking-tight text-white">
                           {{ strtoupper(app()->getLocale() === "bn"
                               ? $player->first_name_bn
            ...
            <div class="mt-16 flex justify-center">
                <nav
                    class="inline-flex items-center gap-2
                           rounded-full bg-white/70 px-3 py-2
                           backdrop-blur-xl
                           shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
        
                    @if ($players->onFirstPage())
                        <span class="px-4 py-2 text-sm text-gray-400 cursor-not-allowed">‹ Prev</span>
                    @else
                        <a wire:click.prevent="previousPage"
                            class="px-4 py-2 text-sm text-gray-600 hover:bg-gray-200 rounded-full transition">
                            ‹ Prev
                        </a>
                    @endif
                    ...
                    @else
                        <span class="px-4 py-2 text-sm text-gray-400 cursor-not-allowed">Next ›</span>
                    @endif
                </nav>
            </div>
