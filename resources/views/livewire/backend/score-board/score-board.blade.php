    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Scoreboard</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider whitespace-nowrap">
                            Match NO.
                        </th>
                        <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider whitespace-nowrap">
                            Match Name
                        ...
                        @if (auth()->user()->can("edit score"))
                            <th
                                class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider whitespace-nowrap">
                                Action
                            </th>
                        @endif
                    </tr>
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($scores as $index => $score)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $scores->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $score->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $score->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $score->venue }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($score->date_time)->format("d M Y, h:i A") }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit score")
                                        <!-- Team 1 -->
                                        <a href="{{ route("edit.score", ["scoreId" => $score->id, "teamId" => $score->team1?->id]) }}"
                                            class="inline-flex items-center px-4 py-1 rounded-full text-sm font-medium
                                             text-green-900 bg-green-200 dark:bg-green-900 dark:text-green-200
                                              transition-colors duration-200"
                                            title="Edit {{ $score->team1?->name_en }} Score">
                                            <!-- Edit Icon -->
                                        ...
                                        <span class="text-red-500 font-medium">vs</span>

                                        <!-- Team 2 -->
                                        <a href="{{ route("edit.score", ["scoreId" => $score->id, "teamId" => $score->team2?->id]) }}"
                                            class="inline-flex items-center px-4 py-1 rounded-full text-sm font-medium
                                              text-amber-900 bg-amber-200 dark:bg-blue-900 dark:text-blue-200
                                               transition-colors duration-200"
                                            title="Edit {{ $score->team2?->name_en }} Score">
                                            <!-- Edit Icon -->
                                            <svg class="h-4 w-4 mr-1.5" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round">
                                        ...
                                        <!-- Match Result -->
                                        <a href="{{ route("match.result", $score->id) }}"
                                            class="inline-flex items-center px-4 py-1 rounded-full text-sm font-medium
                                              text-lime-900 bg-lime-200 dark:bg-lime-900 dark:text-lime-200
                                              transition-colors duration-200"
                                            title="Edit">
                                            <!-- Edit Icon -->
                                            <svg class="h-4 w-4 mr-1.5" xmlns="http://www.w3.org/2000/svg"
                            ...
                            {{-- <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit score")
                                        <a href="{{ route("edit.score", $score->id) }}" title="Edit">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $scores->firstItem() }}</span> to <span>{{ $scores->lastItem() }}</span>
                    of <span>{{ $scores->total() }}</span> entries
                </p>
            </div>
