<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">District</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit district") || auth()->user()->can("delete district"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($districts as $index => $district)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $districts->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $district->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit district")
                                        <button type="button" wire:click="edit({{ $district->id }})" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete district")
                                        <button type="button" wire:click='delete({{ $district->id }})'
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $districts->firstItem() }}</span> to <span>{{ $districts->lastItem() }}</span>
                    of <span>{{ $districts->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap bg-white dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-700 rounded-md shadow-sm -space-x-px"
                    ...
                    @php
                        $current = $districts->currentPage();
                        $last = $districts->lastPage();
                    @endphp
    ...
    @if ($form->isOpen)
        @include("livewire.backend.location.district.fragments.modal")
    @endif
</div>
