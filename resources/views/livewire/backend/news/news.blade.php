    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">News</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
        ...
        <div class="overflow-x-auto">
            <table
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($news as $index => $singleNews)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $news->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ asset(@$singleNews->media?->path) }}" alt="{{ $singleNews->title_en }}"
                                    class="w-12 h-12 object-fit rounded-full border border-gray-200" />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ str()->limit($singleNews->title_en, 30) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($singleNews->date)->format("d M Y") }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $singleNews->status ? "bg-blue-500" : "bg-red-600" }}">
                                    {{ $singleNews->status ? "Active" : "Inactive" }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit news")
                                        <a href="{{ route("edit.news", $singleNews->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete news")
                                        <button type="button" wire:click="delete({{ $singleNews->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $news->firstItem() }}</span> to <span>{{ $news->lastItem() }}</span>
