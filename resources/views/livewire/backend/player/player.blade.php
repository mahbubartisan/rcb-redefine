    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Player</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit player") || auth()->user()->can("delete player"))
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($players as $index => $player)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $players->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">{{ $player->first_name_en ?: "N/A" }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $player->first_name_bn ?: "N/A" }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($player->dob)->format("d M Y") }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $player->playing_role ?: "N/A" }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $player->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit player")
                                        <a href="{{ route("edit.player", $player->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete player")
                                        <button type="button" wire:click="delete({{ $player->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $players->firstItem() }}</span> to <span>{{ $players->lastItem() }}</span>
                    of <span>{{ $players->total() }}</span> entries
                </p>
            </div>
            ...
            {{-- <div>
                <nav class="relative z-0 inline-flex flex-wrap bg-white dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-700 rounded-md shadow-sm -space-x-px"
                    ...
                    @foreach ($players->links()->elements as $element)
                        @if (is_string($element))
                            <!-- Ellipsis -->
                            <span
                                class="relative inline-flex items-center px-4 py-2 border border-gray-600/10 dark:border-gray-600/45 text-gray-500 select-none">
                                {{ $element }}
                            </span>
                        @endif
