<div class="fixed inset-0 z-20 bg-gray-900  bg-opacity-70 transition-opacity overflow-x-hidden overflow-y-auto pointer-events-none"
    ...
    <div class="ease-out transition-all sm:max-w-lg sm:mx-auto p-5 md:p-0 lg:p-0">
        <div
            ...
            <div class="p-5">
                <form wire:submit.prevent="{{ $editMode ? "update" : "store" }}" method="POST" class="space-y-3">
                    <!-- Username -->
                    <div>
                        <label for="name" class="block text-sm text-gray-700 dark:text-gray-400">
                            Username
                        </label>
                        <input type="text" wire:model='name' id="name" placeholder="Enter Name"
                    ...
                    <div>
                        ...
                        <input type="text" wire:model='email' id="email" placeholder="Enter Email"
                            class="w-full mt-2 px-4 py-2.5 bg-white dark:bg-[#132337] text-gray-800 dark:text-gray-300 text-sm border border-gray-200 rounded-md dark:border-[#233A57] focus:outline-none focus:ring-0 focus:border-blue-600 transition duration-300 ease-in-out" />
                        @error("email")
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    <!-- Role -->
                    <div>
                        <label for="roleId" class="block text-sm text-gray-700 dark:text-gray-300">User Role</label>
                        <select wire:model="roleId" id="roleId" name="roleId"
                            class="w-full mt-2 px-3 py-2.5 bg-white dark:bg-[#132337] text-gray-500 dark:text-gray-300 text-sm border border-gray-200 rounded-md dark:border-[#233A57] focus:ring-0 focus:outline-none focus:border-blue-600 transition duration-300 ease-in-out">
                            <option value="">Select Role</option>
                            @foreach ($roles as $role)
                                <option value="{{ $role->name }}" {{ $role->name == $roleId ? "selected" : "" }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
