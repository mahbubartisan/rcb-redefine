<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Role Icon</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
    <div class="my-9 flex justify-end">
        @can("create role icon")
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit role icon") || auth()->user()->can("delete role icon"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($icons as $index => $icon)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $icons->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ asset(@$icon->media?->path ?? "images/user_profile.webp") }}"
                                    alt="{{ $icon->name }}"
                                    class="w-12 h-12 object-cover rounded-full border border-gray-300" />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $icon->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit role icon")
                                        <button type="button" wire:click="edit({{ $icon->id }})" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete role icon")
                                        <button type="button" wire:click='delete({{ $icon->id }})'
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $icons->firstItem() }}</span> to <span>{{ $icons->lastItem() }}</span>
                    of <span>{{ $icons->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap bg-white dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-700 rounded-md shadow-sm -space-x-px"
                    ...
                    <!-- Pagination Numbers -->
                    @foreach ($icons->links()->elements[0] as $page => $url)
    ...
    @if ($form->isOpen)
        @include("livewire.backend.role-icon.fragments.modal")
    @endif
</div>
