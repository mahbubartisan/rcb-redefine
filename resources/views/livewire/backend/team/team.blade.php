    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Team</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit team") || auth()->user()->can("delete team"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($teams as $index => $team)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $teams->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ asset(@$team->media?->path ?? 'images/user_profile.webp') }}" alt="{{ $team->name_en }}"
                                    class="w-12 h-12 object-cover rounded-full border border-gray-300" />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $team->name_en }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $team->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit team")
                                        <a href="{{ route("edit.team", $team->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete team")
                                        <button type="button" wire:click="delete({{ $team->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $teams->firstItem() }}</span> to <span>{{ $teams->lastItem() }}</span>
                    of <span>{{ $teams->total() }}</span> entries
                </p>
            </div>
