<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Video</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit video") || auth()->user()->can("delete video"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider whitespace-nowrap">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($videos as $index => $video)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $videos->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ asset(@$video->media?->path) }}" alt="{{ @$video->media?->name}}"
                                    class="w-12 h-12 object-fit rounded-full border border-gray-300" />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $video->video_code }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $video->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit video")
                                        <button type="button" wire:click="edit({{ $video->id }})" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete video")
                                        <button type="button" wire:click="delete({{ $video->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $videos->firstItem() }}</span> to <span>{{ $videos->lastItem() }}</span>
                    of <span>{{ $videos->total() }}</span> entries
                </p>
            </div>
    ...
    @if ($form->isOpen)
        @include("livewire.backend.gallery.video.fragments.modal")
    @endif
</div>
