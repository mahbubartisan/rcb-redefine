    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Gallery</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit gallery") || auth()->user()->can("delete gallery"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider whitespace-nowrap">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($galleries as $index => $gallery)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $galleries->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">{{ $gallery->title_en }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($gallery->date)->format("d M Y") }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $gallery->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit gallery")
                                        <a href="{{ route("edit.gallery", $gallery->id) }}" title="Edit">
                                            <!-- Edit Icon -->
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $galleries->firstItem() }}</span> to <span>{{ $galleries->lastItem() }}</span>
                    of <span>{{ $galleries->total() }}</span> entries
                </p>
            </div>
