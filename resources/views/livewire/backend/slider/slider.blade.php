    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Sliders</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($sliders as $index => $slider)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $sliders->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $slider->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit slider")
                                        <a href="{{ route("edit.slider", $slider->id) }}" title="Edit">
                                            <!-- Edit Icon -->
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $sliders->firstItem() }}</span> to <span>{{ $sliders->lastItem() }}</span>
                    of <span>{{ $sliders->total() }}</span> entries
                </p>
            </div>
