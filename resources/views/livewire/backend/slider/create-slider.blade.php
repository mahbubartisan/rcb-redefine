    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Create Slider</li>
            </ol>
        </nav>
    </div>

    <div class="bg-white dark:bg-[#132337] rounded-md px-6 pt-6 pb-3 shadow">
        <h2 class="text-[15px] text-gray-900 dark:text-gray-300 font-medium mb-4">Slider Information</h2>
        <form wire:submit.prevent="store" class="space-y-4">
            <div class="grid grid-cols-2 gap-5 mt-5">
                ...
                <div>
                    ...
                    <input type="text" wire:model="form.title" id="title" placeholder="Enter Slider Title"
        ...
        {{-- <form wire:submit.prevent="store" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" wire:model="form.title" id="title" class="w-full border rounded p-2" />
                    @error("form.title")
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="file" wire:model="form.image" id="image" class="w-full border rounded p-2" />
                    @error("form.image")
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror

                    @if ($form->image)
                        <img src="{{ $form->image->temporaryUrl() }}" class="mt-3 w-48 rounded shadow" />
                    @endif
                </div>
            </div>
