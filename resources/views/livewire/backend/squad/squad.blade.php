    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Squad</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
        ...
        <div class="overflow-x-auto">
            <table
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($squads as $index => $squad)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $squads->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ optional($squad->match)->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ optional($squad->tournament)->name_en }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ optional($squad->match)->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ optional($squad->team)->name_en }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $squad->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit squad")
                                        <a href="{{ route("edit.squad", $squad->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete squad")
                                        <button type="button" wire:click="delete({{ $squad->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $squads->firstItem() }}</span> to <span>{{ $squads->lastItem() }}</span>
                    of <span>{{ $squads->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap bg-white dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-700 rounded-md shadow-sm -space-x-px"
                    ...
                    @php
                        $current = $squads->currentPage();
                        $last = $squads->lastPage();
                    @endphp
