<div>
    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Tournament</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
    <div class="my-5 flex justify-end">
        @can("create tournament")
            <a href="{{ route("create.tournament") }}"
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit tournament") || auth()->user()->can("delete tournament"))
                            <th class="px-6 py-3 text-xs text-left font-medium uppercase tracking-wider">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="bg-white dark:bg-[#132337] divide-y divide-gray-200 dark:divide-[#233A57]">
                    @forelse ($tournaments as $index => $tournament)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $tournaments->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <img src="{{ asset(@$tournament->media?->path ?? 'images/user_profile.webp') }}" alt="{{ $tournament->name_en }}"
                                    class="w-12 h-12 object-cover rounded-full border border-gray-300" />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $tournament->name_en }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($tournament->date)->format("d M Y") }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="px-2 py-0.5 text-[13px] rounded-full text-white {{ $tournament->status ? "bg-blue-500" : "bg-red-600" }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    @can("edit tournament")
                                        <a href="{{ route("edit.tournament", $tournament->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @can("delete tournament")
                                        <button type="button" wire:click="delete({{ $tournament->id }})"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-y-3 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $tournaments->firstItem() }}</span> to <span>{{ $tournaments->lastItem() }}</span>
                    of <span>{{ $tournaments->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap bg-white dark:bg-gray-800 text-sm font-medium text-gray-700 dark:text-gray-700 rounded-md shadow-sm -space-x-px"
                    ...
                    <!-- Pagination Numbers -->
                    @foreach ($tournaments->links()->elements[0] as $page => $url)
