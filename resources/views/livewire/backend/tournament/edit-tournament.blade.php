    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Edit Tournament</li>
            </ol>
        </nav>
    </div>
    <div class="bg-white dark:bg-[#132337] rounded-md px-6 pt-6 pb-3 shadow">
        <h2 class="text-[15px] text-gray-900 dark:text-gray-300 font-medium mb-4">Tournament Information</h2>
        <form method="post" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="name_en" class="block text-sm text-gray-700 dark:text-gray-400">
                        Tournament Name (EN)<span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label for="name_bn" class="block text-sm text-gray-700 dark:text-gray-400">
                        Tournament Name (BAN)<span class="text-red-500">*</span>
                    </label>
            ...
            <!-- Teams -->
            <div x-data x-init="new TomSelect($refs.teamSelect, {
                plugins: ['remove_button'],
                create: false,
                persist: false,
                placeholder: 'Select team',
                onChange: function(value) {
                    Livewire.find('{{ $this->getId() }}').set('form.teamId', value);
                }
            });" wire:ignore>
                <label for="teamId" class="block text-sm text-neutral-600 dark:text-neutral-400">
                    Teams<span class="text-red-500">*</span>
                </label>

                <select x-ref="teamSelect" id="teamId" multiple style="display: none" class="tom-select mt-2">
                    @foreach ($form->teams as $team)
                        <option value="{{ $team->id }}"
                            {{ in_array($team->id, (array) $form->teamId) ? "selected" : "" }}>
                            {{ $team->name_en }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div
                    ...
                    @if ($form->photo)
                        ...
                        @endif
                    @elseif($form->tournament->media?->path)
                        <img src="{{ asset($form->tournament->media?->path) }}" alt="Current Image"
                            class="w-full h-full object-cover transition duration-500"
                            :class="{ 'blur-sm scale-105': previewChanging }" />
                    @else
                ...
                <div>
                    <label for="date" class="block text-sm text-gray-700 dark:text-gray-400">
                        Tournament Date<span class="text-red-500">*</span>
                    </label>
